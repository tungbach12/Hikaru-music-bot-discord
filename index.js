...
async function handleCommand(interaction) {
  ...
  switch (commandName) {
    ...
    case 'nowplaying': {
      ...
      interaction.reply({ content: '🎵 Now playing:', embeds: [interaction.message?.embeds?.[0] || new EmbedBuilder().setColor(BLUE).setDescription(`**${track.title}**`)], flags: MessageFlags.Ephemeral });
      break;
    }
...
async function handleButton(interaction) {
  ...
  switch (customId) {
    ...
    case 'stop':
      manager.stop(guild.id);
      interaction.reply({ content: '⏹️ Stopped!', flags: MessageFlags.Ephemeral });
      break;
    case 'shuffle':
      manager.toggleShuffle(guild.id);
      interaction.deferUpdate();
    ...
    case 'autoplay':
      ...
      interaction.reply({ content: `🔄 Autoplay ${queue.autoplay ? 'ON' : 'OFF'}`, flags: MessageFlags.Ephemeral });
      break;
    case 'playlist':
      const list = queue.tracks.slice(0, 10).map((t, i) => {
        const prefix = i === queue.currentIndex ? '▶️' : `${i + 1}.`;
